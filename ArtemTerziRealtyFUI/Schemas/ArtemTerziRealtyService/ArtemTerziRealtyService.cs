	...
	public class RealtyService : BaseService, IReadOnlySessionState
	{
			...
			string entityName)
		{
			...
			Select select = new Select(UserConnection)
				.Column(Func.Sum("ArtemTerziPriceUSD"))
				.From(entityName)
				.Where("ArtemTerziTypeId").IsEqual(Column.Parameter(new Guid(realtyTypeId)))
				.And("ArtemTerziOfferTypeId").IsEqual(Column.Parameter(new Guid(realtyOfferTypeId)))
				as Select;
